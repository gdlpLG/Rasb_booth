...
def gp_log_callback(level, domain, string, data=None):
    ...
    """
    # Python 3: gphoto2 may return str or bytes depending on version
...
class GpCamera(BaseCamera):
    ...
    def capture(self, effect=None, use_timer=False):
        ...
        # Retry capture on transient I/O errors (e.g. -110)
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                LOGGER.warning("Capture attempt %d/%d failed: %s", attempt + 1, max_retries, ex)
