...
class PiApplication(object):
    ...
    def __init__(self, config, plugin_manager):
        ...
        try:
            _disable_physical = config.getboolean('WEB', 'disable_physical_buttons')
        except Exception:
            _disable_physical = False

        if _disable_physical:
            from pibooth.plugins.virtual_buttons import VirtualButtonBoard, VirtualLEDBoard
            self.buttons = VirtualButtonBoard()
            self.leds = VirtualLEDBoard()
            LOGGER.info("Physical buttons disabled")
        else:
